...
class GoogleSearcher:
    ...
    def run(self):
        file_list = self.get_img_list()  # 先获取上传目录的所有文件或者文件夹

        if not file_list:  # 如果文件夹是空的
        ...
        for file in file_list:
            if os.path.isfile(file):
                pass
                # print(file)
                self.simple_file_run(file)
            elif os.path.isdir(file):
                self._download = os.path.join(self._download, os.path.split(file)[1])
                if not os.path.exists(self._download):
                    os.mkdir(self._download)
                for i in os.listdir(file):
                    file_path = os.path.join(file, i)
                    if os.path.isfile(file_path):
                        self.simple_file_run(file_path)
